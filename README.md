# Mojalefa Gideon Nkwana.
### Software Engineer & CyberSecurity Practitioner

I bridge the gap between software development and security. I build tools to automate defenses, analyze vulnerabilities, and construct secure systems.

---

## 🛡️ Focus Areas & Interests
* **Defensive Security:** Log analysis, SIEM engineering, and active threat hunting.
* **Security Automation (DevSecOps):** Integrating SAST/DAST into CI/CD pipelines.
* **Application Security:** Hardening APIs and defending against the OWASP Top 10.

## 🛠️ Technical Toolkit
| Category | Technologies & Tools |
| :--- | :--- |
| **Languages** | C, C++, Python, Bash, JavaScript, and more |
| **Defensive / Ops** | Wazuh, Elastic, Docker, Wireshark, Nmap |
| **Infrastructure** | Linux, Git, GitHub Actions |

## 🎓 Certifications & Labs
* **Certifications:** CompTIA Security+
* **Hands-on Practice:** [My TryHackMe Profile](https://tryhackme.com/p/Emjay00)

## 📫 Let's Connect!
* **LinkedIn:** [linkedin.com/in/username](link)
* **Email:** [your-professional-email@domain.com](mailto:email)
